<template>
  <div>
    <x-header>日数据结果<a slot="right" @click="getUserLogOut">注销用户</a></x-header>
...
<script>
  ...
  export default {
    ...
    methods: {
      generateTable () {
        let url1 = deployUrl + 'api/day/query/' + localStorage.getItem('statisticyear_start') + '/' + localStorage.getItem('statisticyear_end') + localStorage.getItem('selected_enterprise') + '/' + localStorage.getItem('port')
        let _this = this
