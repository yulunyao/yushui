      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            <el-form-item label="阀门执行">
              <span>{{ props.row.valveExecute }} </span>
            </el-form-item>
            <el-form-item label="控制方式">
              <span>{{ props.row.method }} </span>
            </el-form-item>
            <el-form-item label="企业排口">
              <span>{{ props.row.portName }} </span>
            </el-form-item>
            <el-form-item label="阀门名称">
              <span>{{ props.row.valveName }} </span>
            </el-form-item>
          </el-form>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      getValveHistory () {
        ...
        axios.post(url, {
          data: {'psId': localStorage.getItem('valvectrl')},
          ph: {
          ...
          .then(function (response) {
            if (response.data.code === 0) {
              ...
              _this.headerTitle = filteredData.data[0].psName // 操作完成后读取第一条记录中的psName
