<template>
  <div>
    <x-header>阀门控制<a slot="right" @click="getUserLogOut">注销用户</a></x-header>
    <tab>
      <tab-item v-bind:selected="tab1" @on-item-click="onItemClick('fmkzjl')">阀门控制记录</tab-item>
      <tab-item v-bind:selected="tab2" @on-item-click="onItemClick('fmkz')">阀门控制</tab-item>
    </tab>

    <div class="col1" v-if="active_tab == 'fmkz'">
      <group title="请选择需要查找的企业及排口号" label-width="5em">
        ...
        <popup-picker :title="title5" :data="list5" v-model="placeholder5" @on-change="onChange5" :placeholder="('请选择')" show-name></popup-picker>
        <x-input title="控制密码" placeholder="请输入控制密码" placeholder-align="right" text-align="right" type="password" v-model="valvePassword" required></x-input>
      </group>
      ...
      <div v-transfer-dom>
        <confirm v-model="show"
          ...
          <p style="text-align:center;">是否确定执行阀门控制操作</p>
        </confirm>
      </div>
    ...
    <div class="col2" v-if="active_tab == 'fmkzjl'" style="padding-bottom: 50px">
      <div v-transfer-dom>
  ...
  export default {
    ...
    data () {
      return {
        ...
        valvePassword: '',
        list2: [], // 排口下拉框的变量
        list4: [[{'name': '排水口阀门', 'value': '排水口阀门'}, {'name': '收集池阀门', 'value': '收集池阀门'}]],
        list5: [[{'name': '开', 'value': '100'}, {'name': '关', 'value': '0'}]],
        newlist1: [], // 企业下拉框的临时变量
        newlist2: [], // 排口下拉框的临时变量
    ...
    created () {
      this.checkIfLogin()
      this.getValveHistory()
      this.getTableInfoPc()
    },
    methods: {
      getValveHistory () {
        let url = deployUrl + 'api/valve/getall'
        let _this = this
      ...
      onChange4 (val) {
        this.selected_valve = val[0]
      },
      onChange5 (val) {
        this.selected_switch = val[0]
      },
      getSubmission () {
        if (this.selected_port == '' || this.selected_valve == '' || this.selected_switch == '' || this.valvePassword == '') {
          this.$vux.toast.show({
            text: '请填写所有内容',
            type: 'warn',
            position: 'center'
          ...
          let _this = this
          let url = deployUrl + 'api/valve/control'
          axios.post(url, {
            password: this.entrancePassword,
            psId: this.selected_enterprise,
            portId: this.selected_port,
            state: this.selected_switch,
            valveName: this.selected_valve
          })
            .then(function (response) {
      ...
      onConfirm () {
        this.show = false
        localStorage.setItem('valvectrl', this.selected_enterprise) // 门禁记录仅需要psId即可
        this.show2 = true
        this.getValveHistory()
