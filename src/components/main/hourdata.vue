<template>
  <div>
    <x-header>降雨小时数据<a slot="right" @click="getUserLogOut">注销用户</a></x-header>
    ...
    methods: {
      ...
      getSubmission () {
        if (this.selected_port == '') {
          ...
          let url = deployUrl + 'api/hour/query/' + _this.startTime + '/' + _this.selected_enterprise + '/' + _this.placeholder4
          axios.get(url, {
            
          })
            .then(function (response) {
              if (response.data.code === 0) { // 如果返回的状态码为0，则弹出对话框
                _this.$router.push('/hourdata/hourdata_result')
              }
            })
        }
      },
